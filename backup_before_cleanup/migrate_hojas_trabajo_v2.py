...
def main():
    ...
    if check_column_exists('hojas_trabajo', 'id_presupuesto'):
        ...
        try:
            # Agregar la nueva columna con una conexión autocommit
    ...
    print(f"\nResumen: {len(actualizaciones)} hojas actualizadas")
    
    # Crear índice para la nueva columna
    try:
        with engine.connect().execution_options(isolation_level="AUTOCOMMIT") as conn:
            conn.execute(text("CREATE INDEX IF NOT EXISTS idx_hojas_trabajo_presupuesto ON hojas_trabajo (id_presupuesto)"))
